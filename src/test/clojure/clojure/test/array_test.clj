...
(defn nan-or-=
  [a b]
  (or (= a b)
      (and (Double/IsNaN a)                        ;;; isNaN
           (Double/IsNaN b))))                     ;;; isNaN
        ...
        (fn [t]
          `(defn ~(symbol (str "copy-" t "-array"))
            ...
            (let [~'copy ~(list (symbol (str t "-array")) 'a)]
              (dotimes [~'i (alength ~'a)]
                (aset ~'copy ~'i   (aget ~'a ~'i)          ;;;  I think this is a bug.  The tag should be applied to a, not the result of the aget    --- ~(with-meta `(aget ~'a ~'i)
                     ))                                          ;;;     {:tag (symbol (str t "s"))})))
              ~'copy)))
        types)))
